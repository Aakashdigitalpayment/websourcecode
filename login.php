...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['do_login'])) {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        $error = t('Security error. Page refresh गर्नुहोस्।', 'Security error. Please refresh the page.');
    } else {
        $loginId  = trim($_POST['login_id'] ?? '');
        $password = $_POST['password'] ?? '';
        if (!$loginId || !$password) {
            $error = t('इमेल/सदस्यता नम्बर र पासवर्ड आवश्यक छ।', 'Email/member number and password are required.');
        } elseif (function_exists('checkLoginAttempts') && !checkLoginAttempts($loginId, $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0')) {
            $error = t('धेरै पटक गलत प्रयास भयो। कृपया १५ मिनेट पछि पुनः प्रयास गर्नुहोस्।', 'Too many failed attempts. Please try again after 15 minutes.');
        } elseif (function_exists('checkRateLimit') && !checkRateLimit('member_login_pw', 5, 900)) {
            $error = t('धेरै पटक प्रयास भयो। कृपया केही समय पछि पुनः प्रयास गर्नुहोस्।', 'Too many attempts. Please try again later.');
        } else {
            ...
            if (isset($res['error'])) {
                if ($res['error'] === 'pending_approval') {
                    ...
                    $info  = 'rejected';
                    $error = t('❌ तपाईंको दर्ता अस्वीकृत भएको छ।', '❌ Your registration has been rejected.')
                           . (!empty($res['reason']) ? (t(' कारण: ', ' Reason: ') . htmlspecialchars($res['reason'])) : '')
                           . t(' थप जानकारीका लागि कार्यालयमा सम्पर्क गर्नुहोस्।', ' Please contact office for more details.');
                } elseif ($res['error'] === 'renewal_required') {
                    /* Issue #3: 5-year card म्याद सकियो */
                    $info  = 'renewal';
                    $error = t('🔄 तपाईंको Member Card को ५ बर्षे म्याद सकिएको छ। कार्यालयमा सम्पर्क गरी renew गर्नुहोस् — Admin ले approve गरेपछि feri active हुनेछ।', '🔄 Your member card has expired after 5 years. Please contact office for renewal — it will be active again after admin approval.');
                } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['do_member_2fa'])) {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (!is_array($pending) || empty($pending['id'])) {
            $error = t('2FA session समाप्त भयो। पुन: login गर्नुहोस्।', '2FA session expired. Please login again.');
        } else {
            try {
                $db = getDB();
                ...
                if (!$m) {
                    ...
                    $error = t('Member account भेटिएन वा अनुमोदित छैन।', 'Member account not found or not approved.');
                } else {
                    ...
                    if ($mode === 'setup') {
                        ...
                        if ($secret === '') {
                            $error = t('2FA setup secret भेटिएन।', '2FA setup secret not found.');
                        } elseif (!twoFaVerifyCode($secret, $code, 1)) {
                            $error = t('2FA code मिलेन। फेरि प्रयास गर्नुहोस्।', '2FA code did not match. Please try again.');
                        } else {
                        ...
                        if (!$ok) $error = t('2FA code वा backup code मिलेन।', '2FA code or backup code did not match.');
                    }
                ...
                $error = t('2FA verify गर्दा त्रुटि भयो।', 'An error occurred while verifying 2FA.');
            }
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['do_register'])) {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (!$sadasyata) {
            $error = t('सदस्यता नम्बर अनिवार्य छ।', 'Member number is required.');
        } elseif (!$email || !$phone) {
            $error = t('दर्ताका लागि इमेल र मोबाइल दुवै अनिवार्य छन्।', 'Both email and mobile are required for registration.');
        } elseif ($email && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $error = t('मान्य इमेल ठेगाना राख्नुहोस्।', 'Please enter a valid email address.');
        } elseif ($phone && !preg_match('/^[0-9]{7,15}$/', $phone)) {
            $error = t('मान्य मोबाइल नम्बर राख्नुहोस्।', 'Please enter a valid mobile number.');
        } elseif (strlen($password) < 8) {
            $error = t('पासवर्ड कम्तिमा ८ अक्षरको हुनुपर्छ।', 'Password must be at least 8 characters.');
            ...
            $error = t('दुवै पासवर्ड मेल खाएनन्।', 'Passwords do not match.');
        } else {
            $db = getDB();
            $kycMatched = false;
            $kycRow = null;
            try {
                ...
                if ($memberIdCol === '') {
                    $error = t('KYC schema मा Member ID field छैन। Admin ले DB setup/migration चलाउनुहोस्।', 'KYC schema is missing member ID field. Please ask admin to run DB setup/migration.');
                }
            ...
            if (!$kycMatched) {
                $error = t('KYC रेकर्ड फेला परेन। सदस्यता नम्बर + इमेल + मोबाइल KYC विवरणसँग मिलेको छैन।', 'KYC record not found. Member number + email + mobile do not match KYC details.');
            } elseif (($kycRow['status'] ?? '') === 'rejected') {
                $error = t('तपाईंको KYC अस्वीकृत छ। कृपया सहकारीमा सम्पर्क गरी KYC अपडेट गर्नुहोस्।', 'Your KYC is rejected. Please contact cooperative and update KYC.');
            }
            if (!$error && $kycRow) {
                ...
                if ($name === '') $error = t('KYC record मा नाम खाली छ। कृपया KYC update गर्नुहोस्।', 'Name is empty in KYC record. Please update KYC.');
            }
            if ($email) {
                $chk = $db->prepare("SELECT id FROM members WHERE email=? LIMIT 1");
                $chk->execute([$email]);
                if ($chk->fetch()) $error = t('यो इमेल पहिले नै दर्ता भएको छ।', 'This email is already registered.');
            }
            if (!$error && $phone) {
                $chk2 = $db->prepare("SELECT id FROM members WHERE phone=? LIMIT 1");
                $chk2->execute([$phone]);
                if ($chk2->fetch()) $error = t('यो मोबाइल नम्बर पहिले नै दर्ता भएको छ।', 'This mobile number is already registered.');
            }
            if (!$error) {
                $chkS = $db->prepare("SELECT id FROM members WHERE sadasyata_number=? AND sadasyata_number!='' LIMIT 1");
                $chkS->execute([$sadasyata]);
                if ($chkS->fetch()) $error = t('यो सदस्यता नम्बर पहिले नै दर्ता भएको छ।', 'This member number is already registered.');
            }
        }
        ...
        if (!$error) {
            ...
            if (isset($res['error'])) {
                ...
                $tab     = 'login';
                $success = t('✅ दर्ता सफल! KYC विवरणबाट प्रोफाइल स्वतः ल्याइयो। Admin अनुमोदनपछि लगिन गर्न सक्नुहुन्छ।', '✅ Registration successful! Profile was auto-filled from KYC. You can login after admin approval.');
            }
        }
    }
}

$siteName  = getSetting('site_name', t('आकाश बचत तथा ऋण सहकारी संस्था', 'Aakash Savings and Credit Cooperative'));
$siteUrl   = SITE_URL;
...
<title><?php echo htmlspecialchars(t('सदस्य लगिन', 'Member Login')); ?> — <?php echo htmlspecialchars($siteName); ?></title>
...
<a href="<?php echo htmlspecialchars(portalLangToggleUrl(), ENT_QUOTES, 'UTF-8'); ?>" class="auth-lang-toggle notranslate" translate="no" title="<?php echo htmlspecialchars(t('भाषा परिवर्तन', 'Switch language'), ENT_QUOTES, 'UTF-8'); ?>" aria-label="<?php echo htmlspecialchars(t('भाषा परिवर्तन', 'Switch language'), ENT_QUOTES, 'UTF-8'); ?>">
...
<a href="<?php echo $siteUrl; ?>" class="page-back">
    <i class="lucide-icon" aria-hidden="true" data-lucide="arrow-left"></i> <?php echo t('गृहपृष्ठ', 'Homepage'); ?>
</a>
...
<div class="auth-card">
    ...
    <div class="card-header">
        ...
        <?php endif; ?>
        <span class="card-portal-label"><i class="lucide-icon" aria-hidden="true" data-lucide="user-circle"></i>&nbsp;<?php echo t('सदस्य पोर्टल', 'Member Portal'); ?></span>
    </div>

    <div class="card-body">
        <div class="card-title">
            <?php echo $tab === 'register' ? t('नयाँ खाता खोल्नुहोस्', 'Create New Account') : t('सदस्य लगिन', 'Member Login'); ?>
        </div>
        <div class="card-sub"<?php echo $tab === 'register' ? '' : ' hidden'; ?>>
            <?php echo $tab === 'register'
                ? t('सदस्यता नम्बर सहित दर्ता गर्नुहोस् — Admin अनुमोदन पछि लगिन सक्नुहुन्छ।', 'Register with member number — you can login after admin approval.')
                : ''; ?>
        </div>

        <?php if ($error): ?>
            <div class="alert alert-error"><i class="lucide-icon" aria-hidden="true" data-lucide="circle-alert"></i> <?php echo $error; ?></div>
        <?php endif; ?>
        <?php if ($success): ?>
            <div class="alert alert-success"><i class="lucide-icon" aria-hidden="true" data-lucide="check-circle"></i> <?php echo htmlspecialchars($success); ?></div>
        <?php endif; ?>
        <?php if ($info === 'pending'): ?>
            <div class="alert alert-warning">
                <i class="lucide-icon" aria-hidden="true" data-lucide="clock"></i> <strong><?php echo t('अनुमोदन प्रतीक्षामा!', 'Pending Approval!'); ?></strong>&nbsp;
                <?php echo t('तपाईंको खाता Admin को समीक्षामा छ। स्वीकृत भएपछि सूचना पठाइनेछ।', 'Your account is under admin review. You will be notified after approval.'); ?>
            </div>
        <?php endif; ?>
        ...
        <div class="tabs">
                ...
                <i class="lucide-icon" aria-hidden="true" data-lucide="log-in"></i> <?php echo t('लगिन', 'Login'); ?>
            </button>
                <button class="tab-btn <?php echo $tab==='register'?'active':''; ?>" onclick="switchTab('register')">
                <i class="lucide-icon" aria-hidden="true" data-lucide="user-plus"></i> <?php echo t('दर्ता', 'Register'); ?>
            </button>
        </div>
        ...
        <form method="POST" novalidate id="formLogin" style="display:<?php echo $tab==='login'?'block':'none'; ?>;">
            ...
            <div class="field">
                <label><?php echo t('इमेल वा सदस्यता नम्बर', 'Email or Member Number'); ?></label>
                <input type="text" name="login_id" placeholder="<?php echo t('email@example.com वा १२३४५', 'email@example.com or 12345'); ?>" required autofocus>
            </div>
            <div class="field">
                <label><?php echo t('पासवर्ड', 'Password'); ?></label>
                <div class="pw-wrap">
            ...
            <div class="forgot-wrap">
                <a href="<?php echo $siteUrl; ?>member/password-reset-request.php" class="forgot-link"><?php echo t('पासवर्ड बिर्सनुभयो?', 'Forgot password?'); ?></a>
            </div>
            <button type="submit" class="submit-btn">
                <i class="lucide-icon" aria-hidden="true" data-lucide="log-in"></i> <?php echo t('लगिन गर्नुहोस्', 'Login'); ?>
            </button>
            <?php if ($googleUrl || $fbUrl): ?>
            <div class="oauth-divider"><?php echo t('वा यसबाट लगिन', 'Or login with'); ?></div>
            <div class="oauth-row">
                <?php if ($googleUrl): ?>
            ...
            <?php endif; ?>
            <div class="foot-link">
                <?php echo t('खाता छैन?', "Don't have an account?"); ?> <a href="#" onclick="switchTab('register');return false;"><?php echo t('दर्ता गर्नुहोस्', 'Register'); ?></a>
            </div>
        </form>
        ...
        <form method="POST" novalidate id="formRegister" style="display:<?php echo $tab==='register'?'block':'none'; ?>;">
            ...
            <div class="field">
                <label><?php echo t('सदस्यता नम्बर', 'Member Number'); ?> <span class="req-star">*</span></label>
                <input type="text" name="sadasyata_number" id="regSadasyata" placeholder="<?php echo t('जस्तै: १२३४', 'e.g. 1234'); ?>" required>
                <div class="field-feedback" id="fbSadasyata"></div>
            </div>
            <div class="field">
                <label><?php echo t('इमेल', 'Email'); ?> <span class="req-star">*</span></label>
                <input type="email" name="email" id="regEmail" placeholder="email@example.com" required>
                <div class="field-feedback" id="fbEmail"></div>
            </div>
            <div class="field">
                <label><?php echo t('मोबाइल नम्बर', 'Mobile Number'); ?> <span class="req-star">*</span></label>
            ...
            <div class="field">
                <label><?php echo t('पासवर्ड', 'Password'); ?> <span class="req-star">*</span></label>
                <div class="pw-wrap">
                    <input type="password" name="password" id="regPw" placeholder="<?php echo t('८+ अक्षर, A-Z, a-z, 0-9 सहित', '8+ chars with A-Z, a-z, 0-9'); ?>" required minlength="8">
            ...
            <div class="field">
                <label><?php echo t('पासवर्ड पुनः', 'Confirm Password'); ?> <span class="req-star">*</span></label>
                <input type="password" name="confirm" id="regConfirm" placeholder="<?php echo t('माथिको जस्तै', 'Same as above'); ?>" required>
                <div class="field-feedback" id="fbConfirm"></div>
            </div>
            <button type="submit" class="submit-btn" id="regSubmitBtn">
                <i class="lucide-icon" aria-hidden="true" data-lucide="user-plus"></i> <?php echo t('दर्ता गर्नुहोस्', 'Register'); ?>
            </button>
            <div class="foot-link">
                <?php echo t('पहिले नै दर्ता?', 'Already registered?'); ?> <a href="#" onclick="switchTab('login');return false;"><?php echo t('लगिन गर्नुहोस्', 'Login'); ?></a>
            </div>
        </form>
...
document.querySelectorAll('form').forEach(function(form){
    form.addEventListener('submit', function(){
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="lucide-icon fa-spin" aria-hidden="true" data-lucide="loader-2"></i> प्रशोधन हुँदै…';
        setTimeout(function(){
