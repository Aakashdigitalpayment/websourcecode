...
$rows = $db->query("SELECT * FROM hrm_departments ORDER BY sort_order, id LIMIT 200")->fetchAll(PDO::FETCH_ASSOC);
?>
    ...
    <table class="table table-hover mb-0">
      ...
      <?php foreach ($rows as $r): ?>
        <tr>
          ...
          <td class="text-end">
            <button type="button" class="adm-icon-btn adm-icon-btn--edit" title="सम्पादन" aria-label="सम्पादन" onclick='editD(<?= json_encode($r, JSON_HEX_APOS|JSON_HEX_QUOT) ?>)'><i class="fas fa-pen"></i></button>
            <form method="post" class="stf-inline-form" onsubmit="return confirm('हटाउने?');"><?= csrfField() ?><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= (int)$r['id'] ?>"><button type="submit" class="adm-icon-btn adm-icon-btn--delete" aria-label="Delete" title="Delete"><i class="fas fa-trash" aria-hidden="true"></i></button></form>
          </td>
        </tr>
      <?php endforeach; ?>
      </tbody>
