    ...
    function ensureElectionVotingTables(?PDO $db = null): void
    {
        ...
        try {
            ...
            if (function_exists('safeAddColumn')) {
                safeAddColumn($db, 'election_posts', 'designation_id', 'INT NULL DEFAULT NULL AFTER id');
            } else {
                try {
            ...
            $addIdx = static function (PDO $db, string $table, string $idx, string $cols): void {
                if (function_exists('safeAddIndex')) {
                    ...
                    if ($names !== []) {
                        safeAddIndex($db, $table, $idx, $names);
                        return;
                    }
                }
                try {
            ...
            try {
                if (function_exists('safeAddColumn')) {
                    safeAddColumn($db, 'election_vote_submissions', 'source', "VARCHAR(30) NOT NULL DEFAULT 'member_portal' AFTER submitted_at");
                    safeAddColumn($db, 'election_vote_submissions', 'proof_type', "VARCHAR(30) NOT NULL DEFAULT '' AFTER source");
                    safeAddColumn($db, 'election_vote_submissions', 'verified_by_admin_id', 'INT NULL DEFAULT NULL AFTER proof_type');
                    safeAddColumn($db, 'election_vote_submissions', 'verified_by_name', "VARCHAR(120) NOT NULL DEFAULT '' AFTER verified_by_admin_id");
                    safeAddColumn($db, 'election_vote_submissions', 'note', "VARCHAR(500) NOT NULL DEFAULT '' AFTER verified_by_name");
                    safeAddColumn($db, 'election_vote_submissions', 'is_ballot', 'TINYINT(1) NOT NULL DEFAULT 1 AFTER note');
                } else {
            ...
            try {
                if (function_exists('safeAddColumn')) {
                    safeAddColumn($db, 'election_positions', 'post_id', 'INT NULL DEFAULT NULL AFTER cycle_id');
                } else {
...
if (!function_exists('ensureDesignationsTable')) {
    function ensureDesignationsTable(?PDO $db = null): void
    {
        ...
        try {
            ...
            if (function_exists('safeAddColumn')) {
                safeAddColumn($db, 'designations', 'title_np', "VARCHAR(160) NOT NULL DEFAULT ''");
                safeAddColumn($db, 'designations', 'title_en', "VARCHAR(160) NOT NULL DEFAULT ''");
                safeAddColumn($db, 'designations', 'category', "VARCHAR(50) NOT NULL DEFAULT 'committee'");
                safeAddColumn($db, 'designations', 'display_order', 'INT NOT NULL DEFAULT 0');
                safeAddColumn($db, 'designations', 'is_active', 'TINYINT(1) NOT NULL DEFAULT 1');
                safeAddColumn($db, 'designations', 'updated_at', 'TIMESTAMP NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP');
            } else {
